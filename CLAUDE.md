...
- **Cross-container traffic uses the shared `signal` bridge, NOT `host.docker.internal`.** On Linux a container reaching `host.docker.internal:8090` arrives as the docker-bridge gateway IP, which signal-gateway's `127.0.0.1`-only binds refuse. The shared `signal` network (declared with `name: signal` so external consumers reference it by that exact string) is the supported integration path. `signal-gateway-notify` and `signal-gateway-router` are the hostnames other compose stacks should use. `signal-api` stays on `internal` only — its unauthenticated REST surface must never be on the shared network.
